...
if ( ! function_exists( 'benenson_render_countdown_timer_block' ) ) {
	function benenson_render_countdown_timer_block( array $attributes = [] ) {
		...
		$background_color = ! empty ( $attributes['backgroundColor'] ) ? 'background-color:' . $attributes['backgroundColor'] . ';' : '';
		$background_image = ! empty ( $attributes['backgroundUrl'] ) ? 'background-image: url(' . $attributes['backgroundUrl'] . ');' : '';
			...
			<?php } else { ?>
				<div class="coutdownTimer-revealContainer">
					...
					if ( ! empty ( $attributes['revealTitle'] ) ) {
						printf(
							'<h2 class="countdownTimer-title">%s</h2>',
							esc_attr( $attributes['revealTitle'] )
						);
					}

					if ( ! empty ( $attributes['revealContent'] ) ) {
						printf(
							'<p class="countdownTimer-content">%s</p>',
							esc_attr( $attributes['revealContent'] )
						);
					}

					if ( ! empty ( $attributes['revealBtnUrl'] ) && ! empty ( $attributes['revealBtnText'] ) ) {
						printf(
							'<a href="%s" class="btn countdownTimer-btn">%s</a>',
							esc_url( $attributes['revealBtnUrl'] ),
							esc_attr( $attributes['revealBtnText'] )
						);
					}
					?>
				</div>
